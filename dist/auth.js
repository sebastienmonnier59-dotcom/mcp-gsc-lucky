...
import { writeStoredCredentials, CREDENTIALS_PATH } from "./storage.js";
...
function waitForCallback(port, expectedState, authUrl) {
  return new Promise((resolve, reject) => {
    ...
    server.on(
      "error",
      (err) => finish(() => reject(new Error(`Loopback server failed: ${err.message}`)))
    );
...
async function run() {
  ...
  const clientId = (process.env.GSC_LUCKY_CLIENT_ID || "").trim() || await prompt("Paste your OAuth Client ID");
  const clientSecret = (process.env.GSC_LUCKY_CLIENT_SECRET || "").trim() || await prompt("Paste your OAuth Client Secret");
  ...
  if (!tokenJson.refresh_token) {
    throw new Error(
      "Google did not return a refresh token. Revoke access at https://myaccount.google.com/permissions and re-run this command."
    );
  }
  ...
  const sites = (sitesJson.siteEntry || []).map((s) => ({ siteUrl: s.siteUrl || "", permissionLevel: s.permissionLevel || "" })).filter((s) => s.siteUrl !== "");
  if (sites.length === 0) {
    throw new Error(
